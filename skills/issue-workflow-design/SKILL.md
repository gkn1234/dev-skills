---
name: issue-workflow-design
description: 当用户完成 brainstorming 设计、从用户故事继续流程、或明确提到"添加设计"/"design"时使用
---

# 添加设计到 User Story

## Overview

为 User Story 添加设计内容，支持两种模式：简单功能直接写概述，复杂功能链接设计文档。不创建单独的 Design Issue。

## 好的设计标准

| 原则 | 含义 | 检查点 |
|------|------|--------|
| **有明确目标** | 解决什么问题 | 设计目标清晰 |
| **有技术方案** | 如何实现 | 架构/数据流/接口已定义 |
| **有取舍说明** | 为什么这样选 | 备选方案和取舍已记录 |
| **可拆分任务** | 能分解实现 | 可识别出独立的实现任务 |

## 流程

### 1. 检测仓库
- 自动检测 `git remote`
- 失败则询问用户

### 2. 确定 User Story
- 对话上下文中有：直接使用
- 参数指定：`--user-story N`
- 否则：列出 User Story 供选择

### 3. 判断复杂度

Claude 根据功能特点推断复杂度，向用户确认：

**简单功能特征：**
- 单一职责，改动范围小
- 无需新增架构或数据模型
- 实现路径明确，无重大技术决策

**复杂功能特征：**
- 涉及多个模块或系统
- 需要新增架构、数据模型或接口
- 有多种实现方案需要权衡

示例提示：
```
这个功能看起来【简单/复杂】，因为【原因】。
- 简单功能：直接写一段设计概述
- 复杂功能：链接到设计文档

你觉得呢？
```

### 4. 检查设计文档

**复杂功能（有设计文档）：**
- 询问文档路径（如 `docs/plans/xxx-design.md`）
- 自动提取概述（读取文档前 200 字或 Overview 部分）
- 生成文档链接

**简单功能（无设计文档）：**
- 询问设计概述（2-3 句话说明技术方案）
- 或基于对话上下文生成概述，请用户确认

### 5. 质量检查

更新前检查设计内容：
- 有明确目标？
- 有技术方案？
- 可拆分任务？

有问题则提示用户补充，修改后再继续。

### 6. 确保文档已提交（如有文档）

检查设计文档是否已提交到 git：
```bash
git ls-files --error-unmatch {doc_path} 2>/dev/null
```

**未提交：**
- 提示："设计文档需要先提交到 git，以便生成永久链接"
- 引导提交：`git add {doc_path} && git commit -m "docs: 添加设计文档"`
- 获取 commit SHA：`git rev-parse HEAD`

**已提交：**
- 直接获取包含该文件的 commit SHA

### 7. 更新 User Story

使用 `gh issue edit` 更新 User Story 的 body，添加 `## Design` 部分：

```bash
# 获取当前 issue body
gh issue view {issue_number} --json body -q '.body' > /tmp/issue_body.md

# 编辑添加 Design 部分（在适当位置插入）

# 更新 issue
gh issue edit {issue_number} --body-file /tmp/issue_body.md
```

### 8. 引导下一步

"已为 User Story (#N) 添加设计内容。接下来：
- 使用 `issue-workflow-task` 创建实现任务
- 或使用 `superpowers:writing-plans` 编写详细实现计划"

## Design 内容格式

根据复杂度判断，生成以下格式之一：

**简单功能（无设计文档）：**
```markdown
## Design
> 简要设计概述，2-3 句话说明技术方案
```

**复杂功能（有设计文档）：**
```markdown
## Design
> **概述**：简要说明设计目标和方案
>
> 📄 [设计文档](https://github.com/{owner}/{repo}/blob/{commit_sha}/docs/plans/xxx-design.md)
```

> **永久链接**：使用 `{commit_sha}` 指向特定提交，确保分支删除后链接仍可访问。通过 `git rev-parse HEAD` 获取当前 commit SHA。

## Common Mistakes

| 错误 | 示例 | 正确做法 |
|------|------|----------|
| 概述太长 | 复制整个设计文档 | 2-3 句话概括核心方案 |
| 复杂功能没有文档链接 | 只写概述不链接 | 复杂功能必须链接到设计文档 |
| 跳过 brainstorming | 边做边设计 | 复杂功能先完成设计再添加 |
| 设计太大 | 一个设计包含多个功能 | 一个 User Story 对应一个设计 |
| 复杂度判断错误 | 复杂功能当简单处理 | Claude 推断后请用户确认 |

## 示例

用户完成 brainstorming 后："帮我添加设计"

**复杂功能（有设计文档）：**
```
检测到你刚完成了登录系统的设计。

这个功能涉及认证、token 管理、多个接口，看起来是复杂功能。
建议链接到设计文档。你觉得呢？

[用户确认]

设计文档路径是 docs/plans/login-design.md，我提取了概述：
> 基于 JWT 的认证系统，支持 refresh token 轮换，包含登录、登出、token 刷新三个接口。

确认更新 User Story #42 吗？
```

用户确认后 -> 质量检查 -> 确保文档已提交 -> 更新 User Story

**简单功能（无设计文档）：**
```
这个功能是添加一个配置项，改动范围小，看起来是简单功能。
直接写一段设计概述就够了。你觉得呢？

[用户确认]

请提供设计概述（2-3 句话说明技术方案），或者我可以根据讨论内容帮你生成。
```

用户提供或确认概述后 -> 质量检查 -> 更新 User Story

## 上下游关系

| 方向 | 技能 | 说明 |
|------|------|------|
| 上游 | `issue-workflow-user-story` | 先有 User Story |
| 上游 | `superpowers:brainstorming` | 复杂功能建议先完成设计 |
| 下游 | `issue-workflow-task` | 更新 User Story 后引导创建任务 |
| 下游 | `superpowers:writing-plans` | 或编写详细实现计划 |
